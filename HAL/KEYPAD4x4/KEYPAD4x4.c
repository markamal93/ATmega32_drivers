...
u8 KEYPAD_GetKey(void) {
	...
	static u8 Local_u8KPDColumnsArray[KPD_COULMNS_NUM] = { KPD_COLUMN0_PIN,
			KPD_COLUMN1_PIN, KPD_COLUMN2_PIN, KPD_COLUMN3_PIN };
	static u8 Local_u8KPDRowsArray[KPD_ROWS_NUM] = { KPD_ROW0_PIN, KPD_ROW1_PIN,
			KPD_ROW2_PIN, KPD_ROW3_PIN };
	u8 Local_u8RowValue;
	u8 Local_u8ColumnsIterator;
	u8 Local_u8RowsIterator;
	u8 Local_u8PressedKey = KPD_NOT_PRESSED_VAL;
	for (Local_u8ColumnsIterator = 0; Local_u8ColumnsIterator < KPD_COULMNS_NUM;
			Local_u8ColumnsIterator++) {
		GPIO_SetPinValue(KPD_COLUMNS_PORT,
				Local_u8KPDColumnsArray[Local_u8ColumnsIterator], STD_LOW);
		for (Local_u8RowsIterator = 0; Local_u8RowsIterator < KPD_ROWS_NUM;
				Local_u8RowsIterator++) {
			Local_u8RowValue = GPIO_GetPinValue(KPD_ROWS_PORT,
					Local_u8KPDRowsArray[Local_u8RowsIterator]);
			if (Local_u8RowValue == STD_LOW) {
				Local_u8PressedKey =
						Local_u8KPDArray[Local_u8RowsIterator][Local_u8ColumnsIterator];
		...
		GPIO_SetPinValue(KPD_COLUMNS_PORT,
				Local_u8KPDColumnsArray[Local_u8ColumnsIterator], STD_HIGH);
	}
	return Local_u8PressedKey;
}
